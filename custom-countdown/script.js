...
// Getting today's date and setting that as the min attribute in our countdown input form
...
function updateDOM() {
    ...
    countdownActive = setInterval(() => {
        ...
        const { days, hours, minutes, seconds } = timeRemaining(distance);
        const timeArray = [days, hours, minutes, seconds];

        if (distance < 0) {
            // Countdown finished => show the complete view
            countdownEl.hidden = true;
            clearInterval(countdownActive);
            completeElInfo.textContent = `${countdownTitle} finished on ${countdownDate}`;
            completeEl.hidden = false;
        } else {
            // Countdown in progress => show the countdown view
...
function restorePreviousCountdown() {
    ...
    if (localStorage.getItem('countdown')) {
        // Hide input
        inputContainer.hidden = true;
        // Get title/date from localStorage and populate them in the DOM
